...
struct Puzzle {
    ...
    Puzzle(const vector<vector<int>>& b, int h, int m, int ac) : board(b), heuristic(h), moves(m), actualCost(ac) {}
...
vector<Puzzle> generateSuccessors(const Puzzle& puzzle, const vector<vector<int>>& goal) {
    ...
    for (const auto& move : moves) {
        ...
        if (newRow >= 0 && newRow < N && newCol >= 0 && newCol < N) {
            ...
            int ac = puzzle.actualCost + 1;
            successors.push_back(Puzzle(newBoard, h, m, ac));
        }
    }
